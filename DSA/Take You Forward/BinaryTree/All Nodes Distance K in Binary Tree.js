/*

https://leetcode.com/problems/all-nodes-distance-k-in-binary-tree/

https://www.youtube.com/watch?v=i9ORlEy6EsI

https://leetcode.com/problems/all-nodes-distance-k-in-binary-tree/solutions/3748847/simple-javascript-solution-dfs-x-2/

*/

const distanceK = (root, target, k) => {
  // add parent pointers to tree
  const addParents = (node, parent = null) => {
    if (!node) return;
    node.parent = parent;
    addParents(node.left, node);
    addParents(node.right, node);
  };
  // find all vals k distance from target
  const getDist = (node, dist = 0) => {
    // is null node or too far from node
    if (!node || dist > k) return;
    // is already visited
    if (visited.has(node)) return;

    // correct distance away from target, save
    if (dist === k) res.push(node.val);

    // mark node as visited
    visited.add(node);

    // explore all directions from target
    getDist(node.left, dist + 1);
    getDist(node.right, dist + 1);
    getDist(node.parent, dist + 1);
  };

  // DFS twice through tree
  addParents(root);
  const res = [];
  const visited = new Set();
  getDist(target);
  // return all nodes k distance from target
  return res;
};

// my own written

const distanceK1 = (root, target, k) => {
  if (!root) return [];
  if (target === k) return [target.value];

  // add parent to the nodes
  function addParent(node, parent = null) {
    if (!node) return;
    node.parent = parent;
    addParent(node.left, node);
    addParent(node.right, node);
  }

  // get the distance from target
  function getDistance(node, dis = 0) {
    if (!node || dis > k) return;
    if (visited.has(node)) return;
    // get the resultant nodes for distance k
    if (dis === k) {
      result.push(node.val);
    }

    visited.add(node);

    getDistance(node.left, dis + 1);
    getDistance(node.right, dis + 1);
    getDistance(node.parent, dis + 1);
  }

  addParent(root);
  const result = [];
  const visited = new Set();
  getDistance(target);
  return result;
};
